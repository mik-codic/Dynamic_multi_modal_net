...
def extract_spect(video_path, output_dir, fps=None):
    ...
    # Extract frames
    #frame_command = ['ffmpeg', '-i', video_path, "-t","2"]
...
def extract_window_from_spect(path,center,window_size_sec):
    image = cv2.imread(path)
    print("image size__",image)
    y=0
    x=0
    h=300
    w=510
...
def cut_spectro(path,frame_indx,total_frames):
    ...
    center = (width*frame_indx)/total_frames
    bins_per_frame_convers = bins_per_frame*width/total_frames
    # Setting the points for cropped image
    left = bins_per_frame_convers*center-1000*bins_per_frame_convers
    top = 0
    right = bins_per_frame_convers*center+1000*bins_per_frame_convers
    bottom = height
...
for file in os.listdir("/Users/michelepresutto/Desktop/Intership Folder/data_script/data_set/train"):
    ...
    extract_single_frame(dir+file,vid_len/2,total_frames/2,"unpacked_single_frame/"+file)
    extract_spect(dir+file, 'unpacked_single_frame/'+file)
    #extract_window_from_spect(dir+file,vid_len/2,2)
    cutted_spect = cut_spectro('unpacked_single_frame/'+file+"/spectrogram.png",total_frames/2,total_frames)
    print("\n\nsize of the cutted img\n\n",(cutted_spect.size))
